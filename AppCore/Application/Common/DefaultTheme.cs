namespace AppCore.Application.Common
{
    /// <summary>
    /// Valores por defecto de la paleta visual DigiMenu.
    /// Usados al inicializar CompanyTheme en nuevos registros.
    /// </summary>
    public static class DefaultTheme
    {
        public const string PageBackground     = "#F1FAEE";
        public const string HeaderBackground   = "#FFFFFF";
        public const string HeaderText         = "#1D3557";
        public const string TabBackground      = "#1D3557";
        public const string TabText            = "#FFFFFF";
        public const string Primary            = "#E63946";
        public const string PrimaryText        = "#FFFFFF";
        public const string Secondary          = "#457B9D";
        public const string Titles             = "#1D3557";
        public const string Text               = "#1D3557";
        public const string BrowserTheme       = "#FFFFFF";
    }
}
